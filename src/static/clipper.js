...
export default {
  install( Vue ){
    ...
    Vue.prototype.crop  = function () {
      ...
      // Crop
      let crop_loading = document.querySelector('.crop_loading')
      crop_loading.style.display = 'block';
      setTimeout(function () {
        ...
        if( imgData.length < (100*1024) ){
          ...
          //图片上传
          self.postImg( imgData ,crop_loading);
          // 上传到服务启
          self.uploadImg(imgData)
        }else{
          image.onload = function () {
            ...
            //图片上传
            self.postImg( data ,crop_loading );
          }
        }
        
        console.log(self.dataURLtoFile(imgData,'erweima').size,222)
    ...
    //图片上传
    Vue.prototype.postImg = function( imageData ,crop_loading) {
      //这边写图片的上传
      let self = this;
      self.destoried();
      window.setTimeout( function () {
        crop_loading.style.display = 'none';
    ...
    Vue.prototype.compress = function ( img , Orientation ) {
      ...
      //进行最小压缩
      let ndata = canvas.toDataURL( 'image/jpeg' , 2);
      let self = this;
      self.uploadImg(ndata)
    ...
    // 图片上传到服务启
    Vue.prototype.uploadImg = function(imageData){
      let self = this;
      if(imageData.length >  4*1024*1024){
        self.$toast("图片不能大于4M")
      } else {
        self.upload(self.dataURLtoFile(imageData,'erweima.png')).then(data=>{
